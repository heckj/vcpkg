...
option(VCPKG_VERBOSE "Enables messages from the VCPKG toolchain for debugging purposes." ON) # normally OFF
...
if(VCPKG_TARGET_TRIPLET)
    if(VCPKG_VERBOSE)
        message(STATUS "VCPKG-Info: Setting architecture from VCPKG_TARGET_TRIPLET ${VCPKG_TARGET_TRIPLET}")
    endif()
...
elseif(CMAKE_GENERATOR_PLATFORM MATCHES "^[Aa][Rr][Mm]64$")
    ...
    if(VCPKG_VERBOSE)
        message(STATUS "VCPKG-Info: Determining architecture from CMAKE_GENERATOR ${CMAKE_GENERATOR}")
    endif()
    ...
    elseif(CMAKE_GENERATOR MATCHES "^Visual Studio 16 2019$")
        ...
        if(VCPKG_VERBOSE)
            message(STATUS "VCPKG-Info: Fall through - checking cl")
            message(STATUS "VCPKG-Info: CMAKE_SYSTEM_NAME is ${CMAKE_SYSTEM_NAME}")
            message(STATUS "VCPKG-Info: CMAKE_HOST_SYSTEM_NAME is ${CMAKE_HOST_SYSTEM_NAME}")
            message(STATUS "VCPKG-Info: Fall through - checking cl")
        endif()
        ...
        elseif(CMAKE_HOST_SYSTEM_NAME STREQUAL "Darwin" AND DEFINED CMAKE_SYSTEM_NAME AND NOT CMAKE_SYSTEM_NAME STREQUAL "Darwin")
            if(VCPKG_VERBOSE)
                message(STATUS "VCPKG-Info: Determining architecture from Darwin")
            endif()
        ...
        elseif(CMAKE_HOST_SYSTEM_PROCESSOR STREQUAL "x86_64" OR CMAKE_HOST_SYSTEM_PROCESSOR STREQUAL "AMD64")
            # on macOS - Darwin, this is the line that's setting the architecture when running under terminal in Rosetta2
            if(VCPKG_VERBOSE)
                message(STATUS "VCPKG-Info: Determining architecture from CMAKE_HOST_SYSTEM_PROCESSOR ${CMAKE_HOST_SYSTEM_PROCESSOR}")
            endif()
            set(_VCPKG_TARGET_TRIPLET_ARCH x64)
        ...
        elseif(CMAKE_HOST_SYSTEM_PROCESSOR STREQUAL "aarch64")
            ...
            if( _CMAKE_IN_TRY_COMPILE )
                ...
                # macOS M1 Darwin - fall through to here when running on terminal without rosetta
                message(WARNING "Unable to determine target architecture, continuing without vcpkg.")
            endif()
            set(VCPKG_TOOLCHAIN ON)
            return()
